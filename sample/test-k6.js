...
export default function () {
  ...
  group("Front page", function () {
    ...
    if (__ENV.URL_ALERT) {
      res = http.get(
        "http://test.k6.io/?ts=" + Math.round(randomIntBetween(1, 2000))
      );
    } else {
      res = http.get(
        "http://test.k6.io/?ts=" + Math.round(randomIntBetween(1, 2000)),
        { tags: { name: "http://test.k6.io/ Aggregated" } }
      );
    }
    let checkRes = check(res, {
      "Homepage body size is 11026 bytes": (r) => r.body.length === 11026,
      "Homepage welcome header present": (r) =>
        r.body.indexOf("Welcome to the k6.io demo site!") !== -1,
    });
    ...
    group("Static assets", function () {
      let res = http.batch([
        [
          "GET",
          "http://test.k6.io/static/css/site.css",
          {},
          { tags: { staticAsset: "yes" } },
        ],
        [
          "GET",
          "http://test.k6.io/static/js/prisms.js",
          {},
          { tags: { staticAsset: "yes" } },
        ],
      ]);
  ...
  group("Login", function () {
    ...
    let checkRes = check(res, {
      "Users should not be auth'd. Is unauthorized header present?": (r) =>
